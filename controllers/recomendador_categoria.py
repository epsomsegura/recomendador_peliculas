...
class recomendador_categoria:
    # Ruta al dataset
    df = pd.read_csv('src/categories/movies_metadata.csv', low_memory=False, encoding='unicode_escape')
    gen_df = None
    ...
    def __init__(self):
        ...
        if(path.exists('src/categories/categories_metadata.csv')):
            self.gen_df = pd.read_csv('src/categories/categories_metadata.csv', low_memory=False, encoding='unicode_escape')
        else:
            ...
            self.gen_df.to_csv('src/categories/categories_metadata.csv',index=False)
